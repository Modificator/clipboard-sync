...
return fmt.Errorf(envelope.Error.Message)
}
...
st.lastTextHash = item.Hash
st.mu.Unlock()
return client.send(protocol.Envelope{Type: protocol.TypeUpdate, Update: &protocol.ClipboardUpdate{DeviceID: cfg.DeviceID, DeviceName: cfg.DeviceName, Room: cfg.Room, Kind: protocol.KindText, MIMEType: item.MIMEType, Hash: item.Hash, Text: item.Text, Timestamp: time.Now().UTC()}})
}
...
st.lastImageHash = item.Hash
st.mu.Unlock()
return client.send(protocol.Envelope{Type: protocol.TypeUpdate, Update: &protocol.ClipboardUpdate{DeviceID: cfg.DeviceID, DeviceName: cfg.DeviceName, Room: cfg.Room, Kind: protocol.KindImage, MIMEType: item.MIMEType, Hash: item.Hash, DataBase64: base64.StdEncoding.EncodeToString(item.Data), Timestamp: time.Now().UTC()}})
}
...
st.mu.Unlock()
return backend.WriteText(ctx, update.Text)
...
st.mu.Unlock()
return backend.WriteImage(ctx, data, update.MIMEType)
default:
